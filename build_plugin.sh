#!/usr/bin/env bash
#
# build_plugin.sh — package the Evapotranspiration Calculator QGIS plugin
# into a clean ZIP, commit + push the source to git, and drop the ZIP in
# your Downloads folder ready to upload at https://plugins.qgis.org/plugins/add/
#
# Usage:
...
PLUGIN_DIR_NAME="$(basename "$SCRIPT_DIR")"          # e.g. hargreaves_et
VERSION="$(grep -E '^version=' metadata.txt | head -1 | cut -d= -f2 | tr -d '[:space:]')"
ZIP_NAME="Evapotranspiration-Calculator-${VERSION}.zip"
...
# Files/dirs never shipped to end users. build_plugin.sh itself is excluded
# too: it's a dev-only packaging tool, never run by QGIS, and a .sh file
# inside the plugin trips the QGIS plugin repo's "suspicious file type"
# scan (which also flags hidden files, hence .gitignore below).
EXCLUDES=(
    "__pycache__" "*.pyc" ".git" ".gitignore" ".claude" "*.zip"
    "build" "dist" ".pytest_cache" "test" "build_plugin.sh"
)
...
if [ "$DO_GIT" -eq 1 ]; then
  ...
  if git remote get-url origin >/dev/null 2>&1; then
    ...
    echo
    echo ">> No 'origin' remote set. Repo: https://github.com/iliasmachairas/Evapotranspiration-Calculator"
    echo "   Run:"
    echo "     git remote add origin https://github.com/iliasmachairas/Evapotranspiration-Calculator.git"
